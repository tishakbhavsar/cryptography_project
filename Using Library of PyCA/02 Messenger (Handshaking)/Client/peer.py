...
class Peer:
    ...
    def _handshake(self):
        ...
        # Client sends its signing pub, ephemeral pub, and signature first
        ...
        # Receive server's signing pub, ephemeral pub, signature
        server_sign_pub = recv_bytes(self.conn)
        server_eph_pub = recv_bytes(self.conn)
        server_sig = recv_bytes(self.conn)

        # Verify server signature
        if not verify_signature(server_sign_pub, server_eph_pub, server_sig):
            raise Exception("Server signature verification failed")

        # Derive AES and HMAC keys
        self.aes_key, self.hmac_key = perform_key_exchange(self.eph_private_key, server_eph_pub)
